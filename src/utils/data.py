...
def valid_tracks_for_sequence(annotations):
    ...
    for track_id, track in tracks.items():
        ...
        if (
            len(all_image_ids) == len(set(all_image_ids))
            and all_image_ids == sorted(all_image_ids)  # ensure uniqueness
            and len(all_image_ids)  # ensure sortedness
            == all_image_ids[-1] - all_image_ids[0] + 1  # ensure consecutivity
        ):
...
def load_data_3dpw_multiperson(split):
    ...
    for pkl in os.listdir(path_to_data("3dpw", "sequenceFiles", SPLIT_3DPW[split])):
        with open(
            path_to_data("3dpw", "sequenceFiles", SPLIT_3DPW[split], pkl), "rb"
        ) as reader:
        ...
        for actor_index in range(len(annotations["genders"])):
            ...
            for image_index in range(len(joints_2D)):  # range(t1, t2):
                path = path_to_data(
                    "3dpw",
                    "imageFiles",
                    os.path.splitext(pkl)[0],
                    f"image_{str(image_index).zfill(5)}.jpg",
                )

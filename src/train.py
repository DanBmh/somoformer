...
from dataset import batch_process_joints, collate_batch, create_dataset, get_datasets
from evaluate import evaluate_mpjpe, evaluate_vim
from model import create_model
from utils.metrics import bone_dist_mse, keypoint_mae, keypoint_mse
from utils.utils import AverageMeter, create_logger, load_config, load_default_config
...
def compute_loss(
    ...
    loss += config["TRAIN"]["aux_weight"] * sum(
        [keypoint_mse(aux[:, in_F:], out_joints, out_masks) for aux in aux_joints]
    )
    loss /= (
        config["TRAIN"]["aux_weight"] * config["MODEL"]["num_layers"] + 1
    )  # To take average over auxilliary losses
...
def train(config, logger, experiment_name="", args=None):
    ...
    if config["DATA"]["joints"] == "somof":
        ...
        dataset_3dpw_train_segmented = create_dataset(
            "3dpw",
            logger,
            split="train",
            track_size=(in_F + out_F),
        ...
        dataloader_3dpw_train_segmented = dataloader_for(
            dataset_3dpw_train_segmented, config, shuffle=True, pin_memory=True
        )
        dataset_3dpw_val = create_dataset(
            "3dpw",
            logger,
            split="val",
            track_size=(in_F + out_F),
        ...
        dataset_somof_val = create_dataset(
            "somof",
            logger,
            split="valid",
            track_size=(in_F + out_F),
        ...
        dataloader_somof_train = dataloader_for(
            dataset_somof_train, config, shuffle=False
        )
    ...
    if config["MODEL"]["checkpoint"] != "":
        ...
        checkpoint = torch.load(
            os.path.join(config["OUTPUT"]["ckpt_dir"], config["MODEL"]["checkpoint"])
        )
    ...
    for epoch in range(config["TRAIN"]["epochs"]):
        ...
        for i in range(
            train_steps
        ):  # len(dataloader_train)): #enumerate(dataloader_train):
            model.train()
            optimizer.zero_grad()
        ...
        if (epoch + 1) % config["TRAIN"]["val_frequency"] == 0:
            ...
            if config["DATA"]["joints"] == "somof":
                train_vim_somof = evaluate_vim(
                    model,
                    dataloader_somof_train,
                    config,
                    logger,
                    return_all=True,
                ...
                val_vim_somof = evaluate_vim(
                    model,
                    dataloader_somof_val,
                    config,
                    logger,
                    return_all=True,
                ...
                train_vim_3dpw = evaluate_vim(
                    model,
                    dataloader_3dpw_train_segmented,
                    config,
                    logger,
                    return_all=True,
                ...
                for i in range(len(train_mpjpe_3dpw_joint)):
                    writer_train.add_scalar(
                        f"MPJPE/3dpw/j_{i}", train_mpjpe_3dpw_joint[i], global_step
                    )

                val_mpjpe_3dpw_joint = evaluate_mpjpe(
                    model,
                    dataloader_3dpw_val,
                    config,
                    logger,

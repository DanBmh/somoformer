...
class LearnedDoublePositionalEncoding(nn.Module):
    def __init__(
        ...
        self.learned_encoding = nn.Embedding(
            num_joints, d_model // 2, max_norm=True
        ).to(device)
    ...
    def forward(self, x: torch.Tensor, num_people=1) -> torch.Tensor:
        ...
        x[:, :, 0 : half * 2 : 2] = x[:, :, 0 : half * 2 : 2] + self.learned_encoding(
            torch.arange(num_joints).repeat(num_people).to(self.device)
        ).unsqueeze(1)
        x[:, :, 1 : half * 2 : 2] = x[:, :, 1 : half * 2 : 2] + self.person_encoding(
            torch.arange(num_people)
            .repeat_interleave(num_joints, dim=0)
            .to(self.device)
        ).unsqueeze(1)
...
class SoMoFormer(nn.Module):
    def __init__(
        ...
        if location_method == "naive":
            ...
            self.double_id_encoder = LearnedDoublePositionalEncoding(
                nhid, dropout, num_joints=num_joints * 3, device=device
            )
            ...
            self.double_id_encoder = LearnedDoublePositionalEncoding(
                nhid - 3, dropout, num_joints=num_joints * 3, device=device
            )
            ...
            self.double_id_encoder = LearnedDoublePositionalEncoding(
                nhid - grid_emb_size, dropout, num_joints=num_joints * 3, device=device
            )
    ...
    def dct_forward(self, x):
        ...
        tgt_dct = tgt_dct.reshape(-1, *x.shape[1:]).permute(
            2, 1, 0
        )  # (N*J*K, B, dct_in)

        tgt_dct = (tgt_dct - self.dct_m[: self.dct_n]) / self.dct_s[
            : self.dct_n
        ]  # (N*J*K, B, dct_in)
    ...
    def forward(self, tgt, tgt_neck, padding_mask, metamask=False):
        ...
        if self.location_method == "naive":
            ...
            necks_concat = tgt_neck.reshape(B, N, K)[
                :, :, [0, 2]
            ]  # (B, N, 2) Getting the xy coords only
            ...
            pose_grid = (
                (necks_concat * self.grid_len)
                .div(self.grid_len, rounding_mode="floor")
                .long()
            )  # (B, N, 2)
            ...
            grid_emb = grid_emb.transpose(0, 1).repeat_interleave(
                J * K, dim=0
            )  # (N*J*K, B, g_e)
        ...
        #             tgt = torch.cat((tgt, naive_pos_concat), dim=2) # (N*J*K, B, h_dim)
            ...
            necks = (
                tgt_neck.reshape(B, N, K)
                .transpose(0, 1)
                .repeat_interleave(J * K, dim=0)
            )  # (N*J*K, B, K)
        ...
        out, aux_out = self.transformer(
            tgt, mask=None, src_key_padding_mask=tgt_padding_mask
        )
        ...
        if self.three_d_eembedding:
            ...
            aux_out = [
                aux.reshape(-1, B, F, K).permute(0, 3, 1, 2).reshape(-1, B, F)
                for aux in aux_out
            ]
...
class PositionalEncoding(nn.Module):
    def __init__(
        self, d_model: int, dropout: float = 0.1, max_len: int = 5000, device="cuda:0"
    ):
        super().__init__()
        self.dropout = nn.Dropout(p=dropout)
...
class LearnedPositionalEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, num_people=1) -> torch.Tensor:
        ...
        x = x + self.learned_encoding(
            torch.arange(x.size(0)).to(self.device)
        ).unsqueeze(1)
        return self.dropout(x)
...
class CustomDecoder(nn.TransformerDecoder):
    ...
    def forward(self, tgt, tgt_mask=None, tgt_key_padding_mask=None):
        ...
        for i, mod in enumerate(self.layers):
            output = mod(
                output, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask
            )
            aux_output.append(output)
    ...
    if config["MODEL"]["type"] == "somoformer":
        ...
        model = (
            SoMoFormer(
                ...
                seq_len=config["TRAIN"]["input_track_size"]
